    ...
    with tf.io.TFRecordWriter(out_file) as writer:
        ...
        for key, value in ground_truth.items():
            writer.write(record=tf.train.Example(features=tf.train.Features(feature=dict(
                path1=tf.train.Feature(bytes_list=tf.train.BytesList(value=[(in_dir1 / Path(key).with_suffix(".jpg")).encode("utf-8")])),
                path2=tf.train.Feature(bytes_list=tf.train.BytesList(value=[(in_dir2 / Path(key).with_suffix(".jpg")).encode("utf-8")])),
